...
async function fetchAndRenderProducts() {
  ...
  try {
    ...
    // FRONTEND FILTERS
    products = products.filter(p => p.discountedPrice <= maxPrice);
    if (selectedSizes.length > 0) {
      products = products.filter(p => p.variants?.some(v => selectedSizes.includes(v.size)));
    }
    ...
    if (products.length === 0) {
      productGrid.innerHTML = "<p style='padding:20px;'>No products match your filters.</p>";
      return;
    }
